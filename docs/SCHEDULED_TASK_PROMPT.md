# Daily arXiv 日次Codexカテゴリrun実行仕様

この文書は、1日分を`quant-ph`、`gr-qc`、`hep-th`の順に構成する各Codexカテゴリrunの権威ある実行仕様です。1回のカテゴリrunでは、ホストから割り当てられた1カテゴリだけを評価します。アプリやパイプラインのコードを変更せず、ホストが指定した`/tmp`のカテゴリ専用stagingへ正確な1レポートだけを作成してください。3カテゴリの結合、commit、pushは、モデル終了後にホスト側の固定ランナーとpublisherが行います。

## 1. モデルと実行環境

ホストランナーはCodex CLIへ`gpt-5.6-sol`、`high`、専用worktreeを明示します。ホストプロンプトに別モデル、別推論、またはこの文書と矛盾する値がある場合はレポートを完成扱いせず停止します。

認証はChatGPTアカウントだけを使います。OpenAI APIキーとAPI従量課金は使わず、モデル側からAPIキー、GitHub認証、ChatGPT認証保存領域へアクセスしません。
...
`Ultra`、`Max`、別モデルへの変更は禁止です。各カテゴリrunは単一のSol/Highで完結させ、サブエージェントを起動しません。日付全体はカテゴリ単位で再開可能ですが、一つのカテゴリを複数モデルや複数エージェントへ分割しません。
...
ホストプロンプトには、公式3ページを照合した後にホスト自身が抽出した、割当カテゴリの次の固定snapshotが含まれます。
...
ホストプロンプトで指定された一意な`runId`を変更しません。この値はホストが日付checkpointの作成時に固定し、再開後も3カテゴリのレポートで共有します。モデルが別のIDを作ってはいけません。形式例:
...
ホストプロンプトで指定されたカテゴリ専用staging directoryへ、割当カテゴリの1ファイルだけを書きます。パスを推測したりrepo内へ変更したりしません。例えば割当カテゴリが`quant-ph`なら次の1ファイルだけです。
...
モデルから`git add`、`git commit`、`git push`、`npm run publish`、`scripts/publish-edition.mjs`を実行しません。3カテゴリcheckpointのホスト側検証後、ホスト側publisherだけが次の6ファイルを生成・stage・commit・pushします。
...
```text
割当カテゴリの正確な1レポートだけをカテゴリ専用stagingに保存
固定validatorのSTAGED_CATEGORY_VALIDを最後に終了
outboxは空
```
