...
- `titleJa`: 原題の意味に忠実で、日本語として自然に読める題名にします。固有名、記号、標準略語、訳が定着していない専門語は無理に訳しません。
...
`titleJa`、`abstractLines`、`curiosity`、`concept`、`conclusion`、`scoreReasons`、`assessment`は、固有名・数式・標準略語だけを残し、文の骨格を自然な日本語で書きます。一般語を英単語のまま日本語の助詞や「する」へ接続しません。例えば`accessする`は「アクセスする」、`biasされる`は「偏りが生じる」、`playする`は「行う」など、意味に合う日本語または定着した片仮名へ直します。

一文には一つの判断を置きます。「画期的」「非常に重要」「高い独創性」「説得力がある」「分野横断的」など、具体的根拠を伴わない形容を使いません。本文が実証した場合は「示した」「導出した」「観測した」、提案段階なら「提案した」、間接的根拠なら「示唆した」と書き分けます。長い名詞の羅列や不自然な逐語訳を避け、日本語だけでは曖昧になる専門語は原語または一般的な略語を残します。
