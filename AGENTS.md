...
- After Codex exits, deterministically restore the immutable original title, ordered authors, primary category, version, submission type, and canonical URL from the same host-validated metadata before checkpoint validation. Never let this binding change an arXiv ID, rank, score, evidence flag, `sourceUrls` evidence list, or reader-facing evaluation prose; unknown, missing, or duplicate IDs still fail closed.
...
- On identity, count, schema, credential, repository-safety, or publication validation failure, preserve `current.json` and do not publish. Failure to obtain or parse one paper's full text is not a publication failure; keep that paper abstract-only and continue.
...
- Screen every abstract from the host-validated metadata and attempt no more than 12 full texts per category. Treat each PDF/source independently: if its official v1 full text cannot be obtained or safely parsed, keep that paper as `fullTextEvaluated=false` with abstract-only evidence and continue. Final top-10 papers are full-text reviewed whenever available, but an unavailable PDF never defers the category or edition.
...
- Before the final validator, perform one complete self-check of every paper's exact schema keys, score sums, deterministic rank, evidence tuple, source URLs, and natural Japanese, then make at most one batch repair. Do not run the legacy numbered structural or language audit loops. Score-distribution, wording-diversity, and English-token heuristics are advisory quality signals and never block a structurally valid, truthful edition. Run the fixed category validator exactly once as the final command. After `STAGED_CATEGORY_VALID`, exit immediately without another command or filesystem write; the outbox must remain empty.
